...
export default function MyPersonasPage() {
  ...
  useEffect(() => {
    getUserResumes();
    getUserPersonas();
  }, [getUserResumes, getUserPersonas]);

  /* 
  // delete resume not needed here
  ...
  */
  ...
  // Function to ensure proper data structure for PersonaPreview
  const formatPersonaData = (persona) => {
    return {
      personaName: persona.personaName || "",
      professionalTitle: persona.professionalTitle || "",
      yearsOfExperience: persona.yearsOfExperience || 0,
      personaSummary: persona.personaSummary || "",
      uniqueValue: persona.uniqueValue || "",
      careerGoals: persona.careerGoals || "",
      specializations: Array.isArray(persona.specializations)
        ? persona.specializations
        : [],
      keySkills: Array.isArray(persona.keySkills) ? persona.keySkills : [],
      achievements: Array.isArray(persona.achievements)
        ? persona.achievements
        : [],
      industryExpertise: Array.isArray(persona.industryExpertise)
        ? persona.industryExpertise
        : [],
      recommendations: Array.isArray(persona.recommendations)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {savedPersonas.length > 0 && (
          <div className="mb-16">
            ...
            <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-8 mb-8">
              {savedPersonas.map((persona) => (
                <div key={persona._id} className="transition-all duration-300">
                  {/* Persona Preview */}
                  <div className="p-6">
                    {/* Template Preview - A4 dimensions */}
                    <div
                      className="border border-gray-200 rounded-lg bg-white mx-auto shadow-sm relative group cursor-pointer overflow-hidden"
                      style={{ width: "300px", height: "424px" }}
                    >
                      <div className="transform scale-[0.36] origin-top-left w-[278%] h-[1180px] ">
                        <div className="bg-white">
                          <PersonaPreview
                            personaData={formatPersonaData(persona)}
                            template={persona.templateName}
                          />
                        </div>
                      </div>
                        ...
                        <div className="absolute top-4 left-4 text-left">
                          ...
                          <p className="text-sm text-zinc-600 font-bold  px-3 py-1 rounded-md ">
                            {persona.templateName
                              ? `Template ${persona.templateName.replace(
                                  "template",
                                  ""
                                )}`
                              : "Template"}
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  `/persona-builder?template=${persona.templateName}&personaId=${persona._id}`
                                );
                              }}
                        ...
                        <div className="absolute inset-0 bg-black bg-opacity-80 flex items-center justify-center z-30">
                          <div className="bg-white rounded-lg p-6 mx-4 max-w-sm">
                            ...
                            <p className="text-sm text-gray-600 mb-4">
                              ...
                              {persona.personaName}" permanently? This action
                              cannot be undone.
                            </p>
        ...
        {/* Resumes Grid */}
        {savedResumes.length === 0 && savedPersonas.length === 0 ? (
          ...
          <div>
            {/* Create Persona from Resume Section */}
            <h2 className="text-2xl font-bold text-blue-900 mb-6 pt-8">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  `/persona-builder?template=professional&resumeId=${resume._id}`
                                );
                              }}
                              className="bg-gray-600 hover:bg-gray-700 text-white p-2 rounded-full transition-colors shadow-lg"
                              title="Create Persona from Resume"
                            >
                              <svg
                                className="w-4 h-4"
                                fill="none"
